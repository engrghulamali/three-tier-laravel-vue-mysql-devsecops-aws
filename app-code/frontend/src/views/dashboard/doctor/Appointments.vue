...
import DoctorAppointmentsPagination from '../../../components/dashboard/DoctorAppointmentsPagination.vue';
import { useFetchDoctorAppointments } from '../../../stores/fetchDoctorAppointments';
import SearchDoctorAppointmentsPagination from '../../../components/dashboard/SearchDoctorAppointmentsPagination.vue';
import SelectDoctorAppointmentsByStatusPagination from '../../../components/dashboard/SelectDoctorAppointmentsByStatusPagination.vue'; // Adjusted component name
...
onMounted(async () => {
    await axios.get('/read-appointment-notifications');
});
...
const captureSearchQuery = async () => {
    ...
    await allAppointments.fetchSearchedDoctorAppointments(1, searchQuery.value);
...
const appointmentsByStatus = () => {
    ...
    switch (selectedStatus.value) {
        case 'Paid':
            allAppointments.fetchDoctorAppointmentsByStatus('paid');
            break;
        case 'Unpaid':
            allAppointments.fetchDoctorAppointmentsByStatus('unpaid');
            break;
        case 'Completed':
            allAppointments.fetchDoctorAppointmentsByStatus('Completed');
            break;
        case 'Scheduled':
            allAppointments.fetchDoctorAppointmentsByStatus('Scheduled');
            break;
        case 'Canceled':
            allAppointments.fetchDoctorAppointmentsByStatus('Canceled');
            break;
...
async function exportToExcel() {
    ...
    try {
        await axios.get('/fetch-all-doctor-appointments').then((res) => {
...
const deleteAppointment = async () => {
    try {
        const res = await axios.post('/delete-doctor-appointment', {
        ...
        allAppointments.fetchDoctorAppointmentsCount();
    } catch (error) {
        showDeleteConfirmationModal.value = false;
        toast.error('Error', {
            description: 'error',
...
const editAppointment = async () => {
    ...
    try {
        await axios.post('/update-doctor-appointment', {
...
<template>
    <div class="bg-[#F0F5F9]  min-h-screen dark:bg-darkmodebg flex">
        ...
        <div class="flex items-center w-full min-[1025px]:w-[75%] flex-col gap-3">
            ...
            <Breadcrumb first="Dashboard" second="Appointments"
                firstIcon='<i class="fa-solid fa-gauge"></i>' secondIcon='<i class="fa-solid fa-list"></i>' link="/doctor-dashboard" />
            ...
            <div class="w-[80%] bg-white dark:bg-darkmodebg max-[1025px]:w-[90%] p-4 mt-10 rounded-3xl">
                ...
                <!-- Pagination -->
                <SearchDoctorAppointmentsPagination :searchQuery="searchQuery" v-if="searchQuery" />
                <SelectDoctorAppointmentsByStatusPagination v-else-if="selectedStatus" :status="selectedStatus" />
                <DoctorAppointmentsPagination v-else />
            </div>
        </div>
    </div>
</template>
